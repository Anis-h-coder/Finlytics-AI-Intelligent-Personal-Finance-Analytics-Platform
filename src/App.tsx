...
export default function App() {
    ...
    try {
      ...
      if (txRes.ok && txData.success) {
        const isIncomeTx = (t: any) => {
          ...
          if (cat === "income" || cat === "salary" || cat === "deposit" || cat === "paycheck") return true;
          const desc = (t.description || "").trim().toLowerCase();
          return /(salary|paycheck|deposit|payroll|dividend|interest|refund|earnings|bonus|income)/i.test(desc);
        };
        ...
        const mappedTx = txData.transactions.map((t: any) => ({
          ...
          rawMlScore: typeof t.raw_ml_score === "number" && t.raw_ml_score > 0
            ? t.raw_ml_score
            : Math.round((t.anomaly_score || 0.7) * 100),
          recurrenceScore: typeof t.recurrence_score === "number" ? t.recurrence_score : 0,
          merchantNoveltyScore: typeof t.merchant_novelty_score === "number" ? t.merchant_novelty_score : 100,
          amountDeviationScore: t.amount_deviation_score || "LOW",
          temporalDeviationScore: t.temporal_deviation_score || "LOW",
          categoryDeviationScore: t.category_deviation_score || "LOW",
          finalRiskScore: t.final_risk_score ?? Math.round((t.anomaly_score || 0.7) * 100),
          classification: t.classification || (t.is_anomaly === 1 ? "POTENTIAL_ANOMALY" : "NORMAL"),
          recurringType: t.recurring_type || "ONE_TIME_EXPENSE",
          anomalyReason: t.anomaly_reason,
          isIncome: isIncomeTx(t),
        }));
  ...
  const tabs = [
    ...
    { id: "advice", label: "Executive Advisor", icon: BrainCircuit },
  ] as const;
          ...
          <div className="border-t border-slate-200 pt-4 space-y-4">
            <div className="flex flex-col gap-1.5">
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-between px-3.5 py-2 border border-indigo-200 hover:border-indigo-400 text-indigo-700 text-xs font-bold rounded-md bg-indigo-50/50 transition-all cursor-pointer disabled:opacity-50"
              >
                <span className="flex items-center gap-1.5">
                  <Database className="h-3.5 w-3.5 text-indigo-600 shrink-0" />
                  Load Demo Intelligence Data
                </span>
              </button>
          ...
          {loading ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeTab === "copilot" && <Copilot token={token} />}

                {activeTab === "budget" && <BudgetPlanner token={token} />}

                {activeTab === "subscriptions" && <Subscriptions token={token} />}
