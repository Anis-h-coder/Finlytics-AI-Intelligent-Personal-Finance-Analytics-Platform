...
function initializeSchema() {
  ...
  db.exec(schema, (err) => {
    if (err) {
      ...
      // Migration: Ensure new multi-layer anomaly fields exist on transactions table
      const columnsToEnsure = [
        "raw_ml_score REAL DEFAULT 0.0",
        "recurrence_score REAL DEFAULT 0.0",
        "merchant_novelty_score REAL DEFAULT 0.0",
        "amount_deviation_score TEXT DEFAULT 'Low'",
        "temporal_deviation_score TEXT DEFAULT 'Low'",
        "category_deviation_score TEXT DEFAULT 'Low'",
        "final_risk_score REAL DEFAULT 0.0",
        "classification TEXT DEFAULT 'NORMAL'",
        "recurring_type TEXT DEFAULT 'ONE_TIME_EXPENSE'",
        "anomaly_reason TEXT DEFAULT ''"
      ];

      for (const col of columnsToEnsure) {
        db.run(`ALTER TABLE transactions ADD COLUMN ${col}`, () => {
          // Ignore duplicate column errors if already present
        });
      }
...
export async function seedUserTransactions(userId: number) {
  ...
  // Seed sample category budgets
  const sampleBudgets = [
    { category: "Utilities", amount: 2143 },
    { category: "Food", amount: 494 },
    { category: "Shopping", amount: 121 },
    { category: "Transport", amount: 116 },
    { category: "Other", amount: 1326 },
  ...
  const sampleGoals = [
    { name: "Emergency Fund", target: 10000, current: 4200, deadline: nextYearStr, monthly: 500 },
    { name: "Tech Upgrade / Laptop", target: 2500, current: 1200, deadline: sixMonthsStr, monthly: 250 },
    { name: "Vacation Travel Fund", target: 3500, current: 900, deadline: nextYearStr, monthly: 220 },
  ];
  ...
  for (const g of sampleGoals) {
    await dbRun(
      `INSERT INTO financial_goals (user_id, name, target_amount, current_amount, deadline, monthly_contribution)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [userId, g.name, g.target, g.current, g.deadline, g.monthly]
    );
  }

  console.log(`Successfully seeded ${seedTransactions.length} transactions, budgets, and goals for user ID ${userId}`);
}
